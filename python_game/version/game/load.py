...
from game import asteroid, util, resources, meteorite
...
def asteroids(num_asteroids, player_position, batch=None):
    ...
    for i in range(num_asteroids):
        ...
        "小行星运动"
        new_asteroid.rotation = random.randint(0, 360)  #给它一个随机的旋转角度
        new_asteroid.velocity_x, new_asteroid.velocity_y = random.random()*30, random.random()*30
        asteroids.append(new_asteroid)  #每个小行星都被加入到列表中
    return asteroids
...
def meteorites(num_meteorites, player_position, batch=None):
    ...
    for i in range(num_meteorites):
        ...
        pyglet sprites记录它们的位置有两种方法：元组(Sprite.position)和x,y属性， (Sprite.x和Sprite.y)
        '''
        while util.distance((meteorite_x, meteorite_y), player_position) < 100:
